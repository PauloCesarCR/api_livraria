...
const listarAutores = async (req, res) => {
    try {
        ...
        for (const autor of autores) {
            const { rows: livros } = await conexao.query('select * from livros where author_id = $1', [autor.id]);
            autor.livros = livros;
        }
...
const obterAutor = async (req, res) => {
    ...
    try {
        const autor = await conexao.query('select * from autores where id = $1', [id]);
          ...
          const livrosDoAutor = await conexao.query('select * from livros where author_id =$1', [id])
            
          const LivroeAutor = [autor.rows[0],livrosDoAutor.rows]
        if (autor.rowCount === 0) {
            return res.status(404).json('Autor não encontrado');
        }
        return res.status(200).json(LivroeAutor);
...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const excluirAutor = await conexao.query('select * from livros where author_id=$1',[id])

        if (excluirAutor.rowCount > 0){
            return res.status(400).json('Autor não pode ser excluido, pois ele possui livros cadastrados.')
        }
